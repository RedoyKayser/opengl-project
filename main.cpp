...
GLboolean amb=true, spec=true, dif=true; //Ambient, Specular and Diffuse, Light


bool l_on1 = true; //Light 1
bool l_on2 = false; // Light 2
bool l_on3 = false; // Light 3



double spt_cutoff=40; // angle of spot light
...
void table()
{
    ...
    GLfloat leg_width=0.3, leg_height= 0.3, leg_length=8;

    glPushMatrix();
    glScalef(table_width,table_length, table_height); //base
    glTranslatef(-1,-1,-1);
    cube(0.58,0.525,0.482);
    glPopMatrix();
...
void chair()
{
    ...
    GLfloat chair_height = 2, chair_length = 1;

    GLfloat leg_width=0.2, leg_height= 0.2, leg_length=4;


    glPushMatrix();  //upper_base

    glTranslatef((chair_width-leg_width)/2-1.1,(chair_length-leg_length)/2+7,(chair_height-leg_length)/2-2);
    glScalef(chair_width,chair_length*3, chair_height/6); //here length is height value 1*3 =3
    glTranslatef(-1,-1,2.5);
    cube(0.902, 0.078, 0.078);
    glPopMatrix();



    glPushMatrix();
    glScalef(chair_width,chair_length*0.5, chair_height); //base
    glTranslatef(-1,-1,-1);
    cube(0.039, 0.325, 0.541); //color RGB
    glPopMatrix();



    glPushMatrix();
    glTranslatef((chair_width-leg_width)/2+1.1,(chair_length-leg_length)/2-3,(chair_height-leg_length)/2+2.5); //leg1
    glScalef(leg_width,leg_length, leg_height);
    glTranslatef(-1,-1,-1);
    cube(0, 0, 0);
    glPopMatrix();
    ...
    glPushMatrix();
    glTranslatef((chair_width-leg_width)/2-2.5,(chair_length-leg_length)/2-3,(chair_height-leg_length)/2+2.6); //leg2
    glScalef(leg_width,leg_length, leg_height);
    glTranslatef(-1,-1,-1);
    cube(0, 0, 0);
    glPopMatrix();
    ...
    glPushMatrix();
    glTranslatef((chair_width-leg_width)/2+1.125,(chair_length-leg_length)/5-1,(chair_height-leg_length)/2-1.6); //leg3 ( width , height, distance )_
    glScalef(leg_width,leg_length, leg_height);
    glTranslatef(-1,-1,3);
    cube(0, 0, 0);
    glPopMatrix();
    ...
    glPushMatrix();
    glTranslatef((chair_width-leg_width)/2-3.4,(chair_length-leg_length)/5-1,(chair_height-leg_length)/2-1.6); //leg4
    glScalef(leg_width,leg_length, leg_height);
    glTranslatef(-1,-1,3);
    cube(0, 0, 0);
    glPopMatrix();
...
void twelve_full_set_chair_table()
{
    glPushMatrix();
    ...
    glPushMatrix();
    glTranslatef(-30,0,20); //1st row 4th col
    full_set();
    glPopMatrix();
    ...
    full_set();
    glPopMatrix();                    //3rd row 4th col m3x0-2x0
...
void fan_leg()
{
    ...
    glTranslatef(-1,-1,-1);
    cube(1, 1, 1); //fan face white color
    glPopMatrix();
}
...
void fan_face()
{
    ...
    glTranslatef(-1,-1,-1);
    cube(0, 0, 0); //fan face black color
    glPopMatrix();
...
void clock()
{
    ...
    glScalef(0.8,0.01,0.1); // to make it in shape
    glTranslatef(-1,-1,-1); // to make it in shape
    cube(0,0,0); //RGB color
    glPopMatrix();

    glPushMatrix(); //clk_leg2

    glRotatef( C_hr_alpha,0, 1, 0.0 ); //360 deg.
    glScalef(1.5,1,1);
...
void door()
{
    ...
    glScalef(8,15,.5);
    glTranslatef(-1,-1,-1);
    cube(0.329, 0.761, 0.651);
    glPopMatrix();

}

void photo_frame(GLfloat posX, GLfloat posY, GLfloat posZ, GLfloat fsX, GLfloat fsY, GLfloat fsZ)
{
    glPushMatrix();
    glTranslatef(posX, posY, posZ);
    glRotatef(90, 1, 0, 0);
    glScalef(fsX, fsY, fsZ); // Adjust the size accordingly (4, 4, 0.2)
    cube(1, 1, 1);      // Adjust the color accordingly
    glPopMatrix();
}
void frame_set(GLfloat setfX, GLfloat setfY, GLfloat setfZ){
    glPushMatrix();
    glTranslatef(setfX, setfY, setfZ);
    photo_frame(20, -1, 5, 4, 4, 0.3); // 1 frame
    photo_frame(20, -1, 15, 4, 4, 0.3); // 2 frame
    photo_frame(20, -1, -5, 4, 4, 0.3); // 3 frame
    photo_frame(14, -1, 5, 2, 9, 0.3); // 4-1 frame
    photo_frame(9, -1, 5, 2, 9, 0.3); // 4-2 frame
    photo_frame(9, -1, -5, 4.3, 4, 0.3); // 5 frame tx,ty,tz,sx,sy,sz
    glPopMatrix();
}
void room()
{
    ...
    glPushMatrix();//light holder front side
    glTranslatef(3,2,36);
    glRotatef(90,1,0,0);
    glScalef(12,.8,0.8);
    glTranslatef(-1,-1,-1);
    ...
    glPushMatrix();//clock
    glTranslatef(-40,1,31);
    clock();
    glPopMatrix();
     wall(0.247, 0.192,0.478);
    ...
    glPushMatrix();  //back window frame
    glTranslatef(0,1,-20);
    glScalef(30,-0.2,10);
    glTranslatef(-1,-1,-1);
    cube(1.000, 1.000, 1.000);
    glPopMatrix();//back window frame end

     wall(0.247, 0.192,0.478); //back  wall
    glPopMatrix();
...
void light()
{
    ...
    GLfloat l_amb[] = {0.2, 0.2, 0.2, 1.0}; //RGBA color light
    ...
    GLfloat l_pos3[] = {10,60,-30,1.0}; // spot light
...
void animate()
{
    if (bRotate == true) // view eye rotation
    {
        theta += 0.2;
        if(theta > 360.0)
...
int main (int argc, char **argv)
{
    ...
    printf("Use '3' to on/off spot light.\n");
